...
This driver summary is for DBD::Sybase version 0.13.
...
The DBD::Sybase module supports Sybase 10.x and 11.x, and offers
...
DBD::Sybase supports CHAR/VARCHAR/BINARY/VARBINARY, all limited to 255
characters in length.  The CHAR type is fixed length (blank padded).
...
can hold up to 2BG of binary data, including nul characters. The main
...
LongReadLen and LongTrunkOk attributes have no effect. The default
...
operation. Note that 11.9.2 and later servers have various new
...
tables in tempdb. I'm told that this performance problem will be
...
Note that DBD::Pg does not fully parse the statement until
...
DBD::Sybase supports an unlimited number of concurrent database
...
# and Michael Peppler.
# $Id: dbd-sybase.pod,v 1.2 1999/05/16 18:28:04 mpeppler Exp $
